...
* **[Rxflex]** moss: bundled runtime → **v0.7.4**. Supersedes the v0.7.0 dll shipped earlier today, which carried a lock bug that could leave a node unable to classify its own NAT. Adds: **telemetry that actually reports** (the FFI had been silently dropping the Axiom config, so this client had never sent a single event despite being configured for it) with per-attempt connectivity records carrying no addresses; a node can finally tell it is behind symmetric NAT (it only ever sampled one vantage point, and symmetric NAT is *defined* by the mapping differing per destination — so the answer could only ever be "unknown"); and relay is now a fallback rather than a terminus — a pair that lands on a relay keeps being retried for a direct path instead of staying there forever.
* **[Rxflex]** moss: bundled runtime → **v0.7.0**, adding the **overlay discovery layer** — the fix for lobbies being invisible between two players. Once the shared substrate put every room on one network, a client's peers became overwhelmingly strangers who share no channel with it, and subscription state only ever travelled one hop — so two players on the same lobby channel never learned the other existed, the topic mesh never formed, and `Publish` returned `NO_PEERS` into the void. A client now publishes, under the hash of its opaque room topic, a record naming the relays it is attached to; a starved topic resolves its mates through that keyspace and reaches them through their relay. Keys derive from the opaque topic, so a relay holding a record cannot tell which game it belongs to. **Requires upgraded relays** — a lookup can only be answered by a node that can be dialed. Also: the FFI silently dropped the Axiom config, so this client had never actually reported a single error despite being configured for it; now fixed, with per-attempt connectivity telemetry (no addresses).
* **[Rxflex]** moss: bundled runtime → **v0.6.24**, which **reverts a regression that made lobbies invisible between two NAT'd players.** moss v0.6.22/v0.6.23 dropped the UDP session a client keeps with its bootstrap peers (one closed it preferring TCP, the other never opened it). That session is load-bearing for NAT traversal: every moss node answers an `observe` request over it (a built-in STUN — "here is the source address I see you as"), and those observations drive NAT classification and the hole-punch port prediction. Without it, NAT type stays `unknown`, the endpoint gossiped to your peer is the useless **TCP** source port, and the punch never lands — so neither player could see the other's lobby. Rebuild/redeploy `moss.dll` from v0.6.24.
